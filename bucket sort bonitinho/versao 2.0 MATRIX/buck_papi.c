...
int main(int argc, char *argv[]){
    ...
    */
    ...
    #pragma omp parallel for num_threads(NUMM_DE_THREADS)
    for (i = 0; i < QTS_BUCKETZ; i++)
    ...
    //#pragma omp for schedule(static) aqui decidimos nao usar porque tinhamos 
    //resultados piores que tem a ver com a criação de threads(overhead)
  ...
  printf("encontrados %d numeros errados",k);
   ...
   printf ("\nN= %d", QTS_NUMEROS);
   printf ("\nNumero de threads: %d", NUMM_DE_THREADS);
   printf ("\nNumero de BUCKETS: %d\n", QTS_BUCKETZ);
