...
def generateCSR(pemPrivateKey, keyPassphrase, commonName, countryName, stateOrProvinceName, localityName, organizationName, organizationalUnitName):
    ...
    # @Jan/2021 - changed - keyPassphrase must be of type bytes
    errorCode, keyPair = pemPrivateKeyToPKey(
        pemPrivateKey, bytes(keyPassphrase, 'utf8'))
    ...
    # @Jan/2021 - changed - decode result
    return None, OpenSSL.crypto.dump_certificate_request(OpenSSL.crypto.FILETYPE_PEM, x509Request).decode()
...
def signCSR(pemCACertificate, pemCAPrivateKey, keyPassphrase, pemCSR, serialNumber, notBefore, notAfter):
    ...
    # @Jan/2021 - changed - keyPassphrase must be of type bytes
    errorCode, caPrivateKey = pemPrivateKeyToPKey(
        pemCAPrivateKey, bytes(keyPassphrase, 'utf8'))
    ...
    # @Jan/2021 - changed - decode result
    return None, OpenSSL.crypto.dump_certificate(OpenSSL.crypto.FILETYPE_PEM, x509Certificate).decode()
...
def pKeyToPEMPrivateKey(keyPair, keyPassphrase, cipher = DEFAULT_CIPHER):
    ...
    """
    try:
        # @Jan/2021 - changed - keyPassphrase must be of type bytes, and decode result
        return None, OpenSSL.crypto.dump_privatekey(OpenSSL.crypto.FILETYPE_PEM, keyPair, cipher, bytes(keyPassphrase, 'utf8')).decode()
    except:
        return 1, None
...
def pKeyToPEMPublicKey(keyPair):
    ...
    try:
        # @Jan/2021 - changed - decode result
        return None, OpenSSL.crypto.dump_publickey(OpenSSL.crypto.FILETYPE_PEM, keyPair).decode()
    except:
        return 1, None
