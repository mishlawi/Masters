...
#Applied same changes as in the pull request: https://github.com/Demonware/jose/pull/23
...
def verify(jws, jwk, alg, validate_claims=True, expiry_seconds=None):
    ...
    if not verify_fn(
        _jws_hash_str(jws.header, jws.payload), jwk, sig, mod=mod
    ):
...
def hmac_sign(s, jwk, mod=SHA256):
    mac = HMAC.new(jwk['k'], digestmod=mod)
    hmac.update(s)
    return hmac.digest()
