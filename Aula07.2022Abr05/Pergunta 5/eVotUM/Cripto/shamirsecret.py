...
# @Jan/2021 - Changed - using Python3 compliant secret-sharing library
# (https://github.com/shea256/secret-sharing) not yet published on pip3.
# When published, add the secret-sharing library to the requirements and remove
# primes.py, polynomials.py and sharing.py from this directory
from eVotUM.Cripto.sharing import SecretSharer
...
def createSharedSecretComponents(secret, nShares, quorum, uid, pemPrivateKey, keyPassphrase):
    ...
    # @Jan/2021 - secret.encode("hex") to bytes(secret,'utf8').hex()
    shamirSecretShares = SecretSharer.split_secret(
        bytes(secret, 'utf8').hex(), quorum, nShares)
...
def recoverSecretFromComponents(shamirSecretComponents, uid, pemCertificate, allComponents = False):
    ...
    for jwt in shamirSecretComponents:
        ...
        # @Jan/2021 - Added test (errorCode is not None)
    ...
    # @Jan/2021 - hexSecret.decode("hex") to bytes.fromhex(hexSecret).decode()
    secret = bytes.fromhex(hexSecret).decode()
...
def generateSecretTime(secretLength, timeToLive, chars="ABCDEFGHJKLMNPQRSTUVWXYZ23456789"):
    ...
    while (l < secretLength):
        ...
        for c in s:
            # @Jan/2021 - Changed c to chr(c)
            if (chr(c) in chars and l < secretLength):  # printable character
                l += 1
                secret += chr(c)  # @Jan/2021 - Changed c to chr(c)
