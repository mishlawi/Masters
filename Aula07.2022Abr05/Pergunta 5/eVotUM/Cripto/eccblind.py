...
Notice that the ECC curve used is NIST P-256 (defined below, in CURVE).
"""
...
# @Jan/2021 - update to pycryptodome 3.9.9
CURVE = 'p256'  # ECC used is NIST P-256
CURVE_N = ECC._curves[CURVE].order
CURVE_G = ECC._curves[CURVE].G
...
def unpack(s):
    try:
        # @Jan/2021 - update to Python 3
        return list(map(lambda x: hexToInteger(x), s.split(".")))
    except:
        return None
